...
public class IssueService {
	...
	public void createIssue(IssueCreationRequest issueCreationRequest) {
		...
		User userDetailsFromLDAP = getUserDetailsFromLDAP(issueCreationRequest.getResponsiblePerson());
		sendEmail(userDetailsFromLDAP, issue);
		return;
	}
...
//	@Caching(evict = {
//			@CacheEvict(cacheNames = USER, condition = "#userUpdateRequest.getUserType() == 1", allEntries = true),
...
//				() -> new ResourceNotFoundException(messageSource.getMessage(MessageSourceService.NOT_FOUND, "User")));
...
//		User user = this.getUserByUserId(id).orElseThrow(() -> new ResourceNotFoundException(
//				messageSource.getMessage(MessageSourceService.NOT_FOUND, "User Id")));
//
...
//		User user = this.getUserByUserId(userId).orElseThrow(
//				() -> new ResourceNotFoundException(messageSource.getMessage(MessageSourceService.NOT_FOUND, "User")));
...
//		User user = this.getUserByUserId(userId).orElseThrow(() -> new ResourceNotFoundException(
//				messageSource.getMessage(MessageSourceService.NOT_FOUND, "Record")));
...
//		User user = userRepository.findByUserIdAndToken(userId, token).orElseThrow(
//				() -> new ResourceNotFoundException(messageSource.getMessage(MessageSourceService.NOT_FOUND, userId)));
...
//				raiseException(messageSource.getMessage(MessageSourceService.INVALID_STAGE, "2"));
//		}
//		User userFromDB = this.getUserByEmailAndUserId(emailId, userId).orElseThrow(
//				() -> new ResourceNotFoundException(messageSource.getMessage(MessageSourceService.INVALID, "email")));
...
//
//		PasswordResetToken passToken = tokenOpt.orElseThrow(() -> new ApplicationException(
//				messageSource.getMessage(MessageSourceService.LINK_EXPIRED, "Reset password")));
//
...
//			throw new ApplicationException(messageSource.getMessage(MessageSourceService.WRONG_PASSWORD));
...
//		User user = getUserByUserId(userId).orElseThrow(() -> new ResourceNotFoundException(
//				messageSource.getMessage(MessageSourceService.NOT_FOUND, "Security question")));
//
...
//				.orElseThrow(() -> new ResourceNotFoundException(
//						messageSource.getMessage(MessageSourceService.NOT_FOUND, "Login record")));
//
...
//		User user = this.getUserByUserId(forgotPasswordVerifyRequest.getUserId()).orElseThrow(
//				() -> new ResourceNotFoundException(messageSource.getMessage(MessageSourceService.NOT_FOUND, "User")));
...
//			raiseException(messageSource.getMessage(MessageSourceService.INVALID_STAGE, "1"));
//
...
//				.orElseThrow(() -> new ResourceNotFoundException(
//						messageSource.getMessage(MessageSourceService.NOT_FOUND, "Answer")));
...
//		// List<SecurityQuestion> questionAnswer =
//		// this.securityQuestionRepository.findByUserUserId(userName);
...
//				raiseException(messageSource.getMessage(MessageSourceService.PASSWORD_REUSE));
//		}
//
